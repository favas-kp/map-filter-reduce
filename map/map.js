...
function doubleArrayElement(element) {
    return element + element
}

function doubltStringLetter(string) {
    return string.split('').map(doubleArrayElement).join('');
}

const doubleLettersOf = function (strings) {
    return strings.map(doubltStringLetter)
};
...
const charCodesOf = function (strings) {
    return strings.map((string) => string.charCodeAt(0));
};

// extract domain names from ["user1@gmail.com", "admin@yahoo.com"] => ["gmail.com", "yahoo.com"]
function splitDomainName(email) {
    return email.split('@')[1];
}

const domainNamesOf = function (emails) {
    return emails.map(splitDomainName)
};
...
function joinChaptertitleAndPages(chapter) {
    return chapter.title + ': ' + chapter.pageCount + ' pages';
}

function joinChaperInfo(book) {
    return {
        title: book.title,
        chapters: book.chapters.map(joinChaptertitleAndPages)
    }
}

const summarizeBookChapters = function (books) {
    return books.map(joinChaperInfo);
};
...
function joinName(attendee) {
    return attendee.firstName + " " + attendee.lastName;
}

function joinAttendeesName(event) {
    return {
        name: event.name,
        attendees: event.attendees.map(joinName),
    };
}

const getEventAttendees = function (events) {
    return events.map(joinAttendeesName);
};
...
console.log();
console.log(
    doubleLettersOf(["cat", "dog", "bat"]),
    ["ccaatt", "ddoogg", "bbaatt"]
);
console.log();
console.log(
    negatedBooleansOf([true, true, false]),
    [false, false, true]
);
console.log();
console.log(
...
console.log(
    summarizeBookChapters([
        {title: "Book 1", chapters: [{title: "Chapter 1", pageCount: 10}, {title: "Chapter 2", pageCount: 20}]}, 
...
console.log(
    getEventAttendees([
        {name: "Concert", attendees: [{firstName: "John", lastName: "Doe"}, {firstName: "Jane", lastName: "Smith"}]}, 
